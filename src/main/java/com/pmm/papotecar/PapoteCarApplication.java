...
public class PapoteCarApplication {
     ...
     * // CORS
     * 
     * @Bean FilterRegistrationBean<Filter>
     * corsFilter(@Value("${tagit.origin:http://localhost:9000}") String origin) {
     * return new FilterRegistrationBean<Filter>(new Filter() { public void
     ...
     * (HttpServletResponse) res; String method = request.getMethod(); // this
     ...
     * ); if ("OPTIONS".equals(method)) { response.setStatus(HttpStatus.OK.value());
